...
if k > 2
    if step
        ...
        [d, U1, U2, R, K, UU, phi] = lbfgs(u1, u2, s, U1, U2, R, K, UU,...
            mmax, phi);
    else
        % subalgorithm to calculate descent direction via SR1-update
        [d, U1, U2, R, K, UU, phi] = sr1(u1, u2, U1, U2, R, K, UU,...
            S_prev, S, d, i_null_c, mmax, phi);
    end
elseif k == 2
    if step
        % subfunction to calculate descent direction via LBFGS-update
        [d, U1, U2, R, K, UU, phi] = lbfgs(u1, u2, s, [], [], [], [], [],...
            mmax, phi);
    else
        % subalgorithm to calculate descent direction via SR1-update
        [d, U1, U2, R, K, UU, phi] = sr1(u1, u2, [], [], [], [], [],...
            S_prev, S, d, i_null_c, mmax, phi);
    end
end
%% 2nd step: correction
...
if f-f_hat <= -m*delta  % serious step
    ...
    lambda = quadprog(H, r, A, b, Aeq, beq, [], [], [], options_ip);
