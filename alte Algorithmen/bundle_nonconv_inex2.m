function [ f_hat, x_hat, x, i_null  ] = ...
    bundle_nonconv_inex2( x0, fun, subgr_fun)
...
%   set globally? make sure not changable?
       m = 0.05;       % \in (0; 1) ???
       gamma = 2;      % > 0        convexification growth parameter
       tol = 1e-3;     % >= 0       stopping tolerance
       kmax  = 200;    % maximal number of iterations

%   to be chosen, but not globally
       t0 = 1;          % > 0 initial inverse prox-parameter
...
t =  t0;    % initial prox-parameter

%f = zeros(1, kmax + 1);
f_saved = zeros(1, kmax + 1);
f = feval(fun, x0) + 2 * 0.01 *rand(1) - 1;      % size depends on bundle, controlled by J
f_saved(1) = f;  
%f(1) = feval(fun, x0);  
%g = zeros(n, kmax + 1);
g_saved = zeros(n, kmax + 1);
g = feval(subgr_fun, x0) + 2 * 0.01 *rand(n, 1) - 1; %subgradient(subgr_fun, x0); % TODO
...
for k = 1:kmax;
    ...
    % calculate eta in loop like in bundle_nonconv???
...
%     if subprob_flag == 1
%         D1 = [-ones(lJ, 1), s'];
%         B1 = c';  % does not matter for optimal value if f_hat added or not
...
%    end
    ...
    %% 2. descent test
    %f = [f, 0];
    f(lJ + 1) = feval(fun, x(:,lJ + 1)) + 2 * 0.01 *rand(1) - 1;
    f_saved(k + 1) = f(lJ + 1);
    g(:, lJ + 1) = feval(subgr_fun, x(:, lJ + 1)) + 2 * 0.01 *rand(n, 1) - 1; %subgradient(subgr_fun, x);
    g_saved(:, k + 1) = g(:, lJ + 1);
