function [ f_hat, x_hat, delta ] = bundle_bilevel( x0, fun, subgr_fun, kmax, m, t, tol, gamma)
...
w = solve_lower(fun_low, subgr_fun_low, x0); %solution of lower level problem
dw = subgr_w();          % find subgradient of w with respect to x
g = subgr_up();          % subgradient at point bundle point
...
w = solve_lower();   % solution of lower level problem
dw = subgr_w();      % find subgradient of w with respect to x
g = subgr_up();      % evaluate subgradient at new iterate and add information to bundle
