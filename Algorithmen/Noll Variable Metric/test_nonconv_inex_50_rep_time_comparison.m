...
%% function value
% no noise
log_f_nb60_s = abs(log10(f_n(1:15,1,1,1)));
log_f_nb60_b = abs(log10(f_n([20,25,30,35,40,50],1,1,1)));

% constant noise
log_f_nb61_s = abs(log10(sum(f_n(1:15,1,2,:),4)/50));
log_f_nb61_b = abs(log10(sum(f_n([20,25,30,35,40,50],1,2,:),4)/50));

%% time
% no noise
t_nb60_s = sum(t_n(1:15,1,1,:),4)/50;
t_nb60_b = sum(t_n([20,25,30,35,40,50],1,1,:),4)/50;

% constant noise
t_nb61_s = sum(t_n(1:15,1,2,:),4)/50;
t_nb61_b = sum(t_n([20,25,30,35,40,50],1,2,:),4)/50;

%% iterations
% no noise
k_nb60_s = k_n(1:15,1,1,1);
k_nb60_b = k_n([20,25,30,35,40,50],1,1,1);

% constant noise
k_nb61_s = ceil(sum(k_n(1:15,1,2,:),4)/50);
k_nb61_b = ceil(sum(k_n([20,25,30,35,40,50],1,2,:),4)/50);

%% SR1
%% function value
% no noise
log_f_ns60_s = abs(log10(f_n(1:15,2,1,1)));
log_f_ns60_b = abs(log10(f_n([20,25,30,35,40,50],2,1,1)));

% constant noise
log_f_ns61_s = abs(log10(sum(f_n(1:15,2,2,:),4)/50));
log_f_ns61_b = abs(log10(sum(f_n([20,25,30,35,40,50],2,2,:),4)/50));

%% time
% no noise
t_ns60_s = sum(t_n(1:15,2,1,:),3)/50;
t_ns60_b = sum(t_n([20,25,30,35,40,50],2,1,:),3)/50;

% constant noise
t_ns61_s = sum(t_n(1:15,2,2,:),3)/50;
t_ns61_b = sum(t_n([20,25,30,35,40,50],2,2,:),3)/50;

%% iterations
% no noise
k_ns60_s = k_n(1:15,2,1,1);
k_ns60_b = k_n([20,25,30,35,40,50],2,1,1);

% constant noise
k_ns61_s = ceil(sum(k_n(1:15,2,2,:),3)/50);
k_ns61_b = ceil(sum(k_n([20,25,30,35,40,50],2,2,:),3)/50);
