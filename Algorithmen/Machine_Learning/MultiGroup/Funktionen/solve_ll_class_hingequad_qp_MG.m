function [ Wb ] = solve_ll_class_hingequad_qp_MG( X, Y, C )
...
for t = 1:T
    ...
    Xi = reshape(bsxfun(@times,ones(J*max((t-1),1),G),C),J*max((t-1),1),1);
    H = spdiags([ones(feat,1);0;2*Xi],0,feat+1+J*max((T-1),1),feat+1+J*max((T-1),1));
