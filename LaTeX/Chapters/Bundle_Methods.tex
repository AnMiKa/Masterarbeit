...
\begin{align}
	&m_{k+1}(\hat{x}^k+d) \geq f(\hat{x}^{k+1})-e^{k+1}_{k+1}+\left\langle g^{k+1},d \right\rangle \quad \forall d \in \R^n \label{model_update_1} \\
...
\vspace{1ex}
\textbf{Algorithm \ref{sec_conv_ex}.1: Basic Bundle Method}
\vspace{1ex}
\hrule
\vspace{1ex}
...
\begin{enumerate}
	...
	\item Set
		\begin{equation*}
			\begin{split}
				G^k = \sum_{j \in J_k}{\alpha^k_j g^k_j}, \quad E_k = \sum_{j \in J_k}{\alpha^k_j e^k_j}, \quad \text{and} \quad  \delta_k =  E_k + \frac{1}{t_k} d_k^2
			\end{split}
		\end{equation*}
...
In the case \(X = R^n\) the sequence \(\{\hat{x}^k\}\) can be unbounded. In this case bounding \(t_k \leq t_{max} < \infty\) for all  \(k\) preserves the convergence proof \cite[Theorem 3.2.2]{Hiriart-Urruty1993}.
