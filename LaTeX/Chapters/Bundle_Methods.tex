...
To provide an easier understanding of the proximal bundle method in \cite{Hare2016} and stress the most important ideas of how to deal with nonconvexity and inexactness first a basic bundle method is shown here. \\ \textcolor{red}{link to chapter?}
...
\begin{equation}
	\min \tilde{m}_k(x) =  m_k(x) + \frac{1}{2 t}\|x-x^k\|^2 \quad \text{s.t.} \quad x \in X
...
\begin{equation}
	\delta_{k} = f(\hat{x}^k) - \tilde{m}_k(x^{k+1}) +a_k \geq 0
\label{nom_dec}
\end{equation}
...
Subproblem (\ref{stabil_model_fun}) then writes as
\begin{equation}
	\min_{\hat{x}^k+d \in R^n, \xi \in \R} \xi + \mathbb{I}_X + \frac{1}{2t_k}\|d\|^2 \quad \text{s.t.} \quad {g^j}^{\top}d - e^k_j - \xi \leq 0, \quad j \in J_k
\label{sub_prob_fin}
\end{equation}
