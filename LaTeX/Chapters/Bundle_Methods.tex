...
To tackle this problem a penalty term is introduced to the subproblem. It then reads
...
The regularization term can be motivated and interpreted in many different ways, c.f. \cite{Hiriart-Urruty1993}. From different possible regularization terms the most popular in bundle methods is the penalty-like regularization used here.
...
Define first the \emph{model decrease} \(\delta^M_k\) which is the decrease of the model for the new iterate \(x^{k+1}\) compared to the function value at the current stability center \(\hat{x}^k\)
...
Besides the model decrease other forms of decrease measures and variations of these are possible. Some are presented in \cite{Hiriart-Urruty1993} and \cite{Oliveira2014}.

\subsubsection{Subproblem Formulations}
...
Subproblem (\ref{stabil_model_fun}) then reads 
\begin{equation}
...
There exist a subgradient \(\nu^k \in \partial \mathtt{i}_{X}(x^{k+1})\) and Lagrangian multipliers \(\alpha_j, ~j \in J^k\) such that
...
\begin{align}
	...
	& \Langle g^j,d^k\Rangle - e^k_j - \xi_k \leq 0, \label{KKT_4}\\
...
\begin{lemma}\cite[Theorem 6.68]{Geiger2002}
...
This means that in the unconstrained case \(G^k \in \partial_{E_k} f(\hat{x}^k)\). So driving \(\|G^k\|\) and \(E_k\) close to zero results in some approximate \(\varepsilon\)-optimality of the objective function.
...
where (\ref{agg_subgr}) and (\ref{agg_err}) were used, shows that the new \(\delta_k\) is only a little variation of the model decrease \(\delta^M_k\).
...
\begin{align}
	&m_{k+1}(\hat{x}^k+d) \geq f(\hat{x}^{k+1})-e^{k+1}_{k+1}+\left\langle g^{k+1},d \right\rangle \label{model_update_1} \\
...
This can however impair the speed of convergence if the bundle is kept too small and provides hence less information about the objective function \cite{Oliveira2014a}.
...
\begin{enumerate}
	...
	\item Select the new bundle index set \(J_{k+1}\), calculate \(e_j^{k+1}\) for \(j \in J_{k+1}\)	and update the model \(m_k\).
\end{enumerate}
\vspace{1ex}
\hrule
...
In the case \(X = R^n\) the sequence \(\{\hat{x}^k\}\) can be unbounded. In this case bounding \(t_k < \infty ~ \forall k\) preserves the convergence proof \cite[Theorem 3.2.2]{Hiriart-Urruty1993}.

In general it can be shown that if \(f\) posses global minima and the basic bundle algorithm generates the sequence \(\{\hat{x}^k\}\) this sequence converges to a minimizer of problem (\ref{min_prob_basic}) (c.f \cite{Hiriart-Urruty1993}).
