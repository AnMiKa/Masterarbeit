...
\[ f(\hat{x}^k) - f(x^{k+1}) \geq \theta \delta^M_k, \quad \theta \in (0,1) \]
...
If \(X\) is a polyhedron this is a quadratic optimization problem that can be solved using standard methods of nonlinear optimization. It should however be observed that the matrix of the quadratic part is only positive semidefinite because it does not have full rank.
