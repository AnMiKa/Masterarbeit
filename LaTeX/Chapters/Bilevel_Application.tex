...
\begin{theorem}[{c.f. \cite[chapter 3.4]{Koenigsberger2002}}]
	Let \(F: U \times V \to Z \) be a \(\mathcal{C}^1\) mapping, \((x_0,y_0) \in U\times V\)  and \(F(x_0,y_0) = 0\). If the matrix \(\frac{\partial}{\partial y}F(x_0,y_0)\) is invertible, there exist neighborhoods \(U_0\subset U\) of \(x_0\) and \(V_0\subset V\) of \(y_0\) and a continuously differentiable mapping \(f:U_0 \to V_0\) with 
	\begin{equation*}
...
In special cases the gradient can be calculated explicitly.
For these calculations to be possible the inner level loss function must yield a linear optimality condition in \(w\). This is for example the case for SVM loss functions with a squared one- or two-norm as given in problem (\ref{SVM_2}).
...
It is therefore assumed that \(\mathcal{L}_{low}\) is once continuously differentiable in \(w\). Contrary to the exemplary calculations from above in practice the second derivative \(\frac{\partial^2}{\partial w \partial \lambda} \mathcal{L}_{low}(w(\lambda),\lambda)\) is not existent in this form, but a set of subgradients.
...
The \emph{'partial' subdifferential} of a function \(f(a^*,\bar{b},\bar{c},...)\) at the point \(a^*\) with respect to one variable \(a\) when all other variables are fixed is denoted by 
\begin{equation*}
