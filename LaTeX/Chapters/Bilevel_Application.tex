...
The two objecective functions \(F\) and \(f\) map from \(\R^n\times\R^k\) into \(\R\) and the constraint functions \(G\) and \(g\) map from \(\R^n\times\R^k\) into \(\R^L\) and \(\R^l\) respectively. 
...
For a continuously differentiable function \(f:\R^n \times \R^m \to \R\) we denote the \emph{gradient} at \((\bar{x},\bar{y})\) by \(\nabla f(\bar{x},\bar{y}) \in \R^{n+m}\). The partial gradient with respect to \(x\) is indicated by \(\nabla_yf(\bar{x},\bar{y}) \in \R^m \). %If \(f\) is twice continuously differentiable the \emph{Hessian matrix} at the point \((\bar{x},\bar{y})\) is given by \(\nabla^2 f(\bar{x},\bar{y}) \in \R^{n+m \times n+m}\).
For a continuously differentiable vector valued function \(F:\R^n \times \R^m \to \R^k\) the \emph{Jacobian matrix} is given by \( \mathcal{J}F(\bar{x},\bar{y}) \in \R^{k \times n+m}\). The partial Jacobian with respect to the variable \(y\) is denoted \(\mathcal{J}_y F(\bar{x},\bar{y}) \in \R^{k \times m}\).

Finally let \(M \in \R^{n\times m}\) be a matrix and \(I \subset \{1,...,n\}\) an index set. Then the expression \(M_{I}\) denotes the matrix that consists of the rows of \(M\) corresponding to the indices in the set \(I\). %For a vector \(v \in \R^n\) the vector \(v_{I}\) consists of the components of \(v\) corresponding to the set \(I\).
...
Next assumptions (A2) and (A3) are verified. In order to do this consider the lower level problem in its unconstrained formulation.
...
Problem (\ref{low_QP}) has a strictly convex function and therefore a unique global minimizer. Denoting the vector of all slack variables \(\xi_i\) with \(\xi\) this minimizer is given by \((\tilde{w}^*,\xi^*)^{\top}\). It depends on the vector of hyper-parameters \(C\).
...
for a given \(C\), where \(L(C,\tilde{w},\xi,\mu) = \frac{1}{2}(\tilde{w}^{\top},\xi^{\top})H(C)(\tilde{w}^{\top},\xi^{\top})^{\top}+\Langle\mu,\left(A (\tilde{w}^{\top},\xi^{\top})^{\top} - b\right)\Rangle\) is the Lagrangian of the problem and \(\mu^*\) the multiplier corresponding to the inequality constraints.
...
This new GE is very convenient due to the simple structure of the cone but it depends now also on the Lagrange multiplier \(\mu\). Therefore this multiplier also has to be unique at the solution \(\tilde{w}^*,\xi^*\).
...
The set \(I_+\) also depends on the multiplier \(\mu\). In cases where \(\mu\) is not unique, this has to be indicated by writing \(I_+(\tilde{w},\xi,\mu)\).  As it is shown in the following that for the presented problem the multiplier \(\mu\) is always unique, it is however omitted. 

For the optimal Lagrange multiplier \(\mu^*\) to be unique the \emph{linear independence constraint qualification} (LICQ) has to hold in the minimum \cite[Theorem 4.8, p. 82]{Outrata1998}, \cite[Theorem 1, p. 3]{Wachsmuth2013}.

In the present case, where there are only inequality constraints, the LICQ holds at a point \((\tilde{w},\xi)\) if the rows of the matrix \(\mathcal{J}_{(\tilde{w},\xi)}\left(A(\tilde{w},\xi)^{\top} - b\right) = A\) that correspond to the indices in \(I(\tilde{w},\xi)\) are linearly independent \cite[p. 82, 96]{Outrata1998}.
...
Because all components of the vector \((C_1,...,C_1,...,C_G,...,C_G)^{\top}\) are greater than zero, this matrix is positive definite and as such strictly copositive on the whole \(\R^{feat+1+nd}\).
This means that all necessary conditions are fulfilled in order to compute a subgradient with respect to \(C\) of the upper level objective function \(\mathcal{L}_{upp}(\tilde{C},\xi(C))\) in the way presented in \cite{Outrata1998}.
...
To ensure a proper choice of the index set \(M_i(\tilde{w},\xi)\) the book offers a theorem and a corollary that are applicable in the given context.
...
\begin{corollary}[c.f. {\cite[Corollary 7.12, p. 139]{Outrata1998}}]
	...
	\[  \ker\left(\mathcal{J}_C\left(H(C)(\tilde{w},\xi)^{\top}\right)^{\top}\right) \cap \ker\left(A_{I_+}\right) = \{0\} .\]
