...
There are five data sets used from which two are real world data sets available on the UCI Machine Learning repository \cite{Lichman2013} and two synthetic data sets.
The synthetic data sets were particularly designed for the optimization of the hyper-parameter \(C\).

A correct choice of the hyper-parameter is especially important in the case were the data of the different folds is especially sensitive to overfitting.
This is the case in the following situation: The data of each of the different folds in nearly separable, but only with a rather small margin. Additionally, the optimal hyperplanes for the different folds have different directions from the optimal hyperplane, because they are only fitted to minimize the error on the training set.
...
\begin{figure}[ht]
	...
	\caption[Overfitting]{The figure on the top left shows the \texttt{synsmall} data set. The solid line is the optimal decision line, with the margin indicated as dotted lines on either side of it. The unregularized decision boundaries for the single folds are drawn with dashed lines. \\
	The other three plots show the training data corresponding to the first to third fold of the \texttt{synsmall} set and the corresponding unregularized decision lines.}
	\label{fig_synsmall}
\end{figure}
...
It can also be seen, that the optimal hyperplane found if \(C\) is chosen optimally is only similar to two of the dashed lines. The line found for the second fold is very different to those.

The situation shown in the plots can also be expressed n numbers. Remember that the margin is proportional to the inverse of the norm of the vector \(w\). The matrix containing the vectors for the different folds is given as

\[ W = \begin{pmatrix} 116.7223 & 10.2238 & 26.9331 \\
                      -103.4875 & -19.8813& -26.6014\end{pmatrix} \text{ and the bias } b = \begin{pmatrix} 20.2872 & 0.7818 & 0.6791	\end{pmatrix}. \]
...
For linear classification, the danger of overfitting is rather small. This is also indicated that from a group of different real world data sets only the two used here provided better results with the regularization term than without it.
...
All data sets were standardized to have a zero norm and unit standard deviation. Then all data sets were split up into three folds of equal size. It is shown in \cite[p. 47]{Kunapuli2008} that this is a reasonable number of folds.
...
These only work with the function values and do not have to have a derivative or subgradient provided by the user.
...
Here the bundle methods are compared to the \texttt{fminbnd} algorithm \texttt{fminsearch}.
...
The starting values are taken from the set \( C_0 \in \{10^{-5},0.01,1,20,100\}\) respectively. The \texttt{fminbnd} algorithm does not need any starting value.
Compared are the unscaled version of the hingequadloss and a scaled version with scaling factor 100.
The stopping tolerance of all methods is set to \(\texttt{tol} = 10^{-6}\).
The results are presented in tables \textcolor{red}{...}

For this comparison the lower level problem as well as the adjoint problem for the bundle methods are solved with a tolerance of \(10^{-15}\) for the constraints and optimality condition.
...
\textcolor{red}{tables (eine Auswahl)}

The MATLAB methods \texttt{fminsearch} and \texttt{fminbnd} solved the problem very accurately and reached the same values to the order depicted here, this is why the results are presented in only one row.
One can see, that \texttt{fminsearch} arrived at the same minimizer for every starting value. It needed however often more time than \texttt{fminbnd}. This is particularly notable for the data set \texttt{synbig}, where \texttt{fminbnd} needs less than half of the solution time of \texttt{fminsearch}.
Both algorithm reach the minimizers that can be identified from the plots to an accuracy of about \(10^{-6}\). For the \texttt{synsmall} data set they find the global minimum instead of the local one.

Both bundle algorithms performed rather badly compared to the MATLAB routines. Although the bundle algorithms can use subgradient information which the MATLAB routines lack.
It can be observed, that for the \texttt{cancer} data set and the \texttt{synsmall} data set, the bundle methods solve the task faster, but the solutions are never as exact as the ones obtained by the MATLAB solvers.
...
In terms of accuracy the results achieved for the different starting values vary a lot more than for the MATLAB solvers. Often for more than half of the chosen starting values the methods even stopped after the first step.
For the \texttt{synsmall} data set it depends on the starting value, weather the local or the global minimum is found.

The bundle algorithms \ref{sec_nonconv_inex}.1 and \ref{sec_variable_metric}.1 show similar results. Algorithm  \ref{sec_nonconv_inex}.1 needs slightly less steps and therefore also less time than algorithm  \ref{sec_variable_metric}.1 for most of the optimization tasks. Still the values are comparable. 
The method \ref{sec_variable_metric}.1 is a bit more stable in the results for the minimizer \(C_{min}\). Both algorithms reach an accuracy of about \(10^{-3}\) of the minimizer compared to the MATLAB routines.

For the objective function scaled with 100 both bundle algorithms stop less often after the first step. On the other hand for this scaling there are tree times, when a bundle algorithm stopped due to the number of steps. This does not happen with the unscaled objective.
The accuracy of the optimal found function value lies around \(10^{-6}\) (\(10^{-4}\) for the objective scaled with 100) if a correct minimizer was found.
...
Next we compare different stopping tolerances of the bundle algorithm.
For this comparison only the starting value \(C_0 = 1\) and the scaled objective function are taken, as this combination provided decent results in the above experiments.
The lower level and adjoint problems are still solved to an accuracy of \(10^{-15}\). The accuracy of the overall method is increased to \(10^{-7}\) and \(10^{-10}\).
...
\textcolor{red}{The full results can be found in table ... in the Appendix???}
...
The other parameters are set in a way that achieved the best results in the comparison before.
...
\textcolor{red}{syn small possesses two local minima> one at ca5 and one at ca 6}
...
\begin{table}[H]%
    ...
    Data set & algorithm &  \\
		...
		\texttt{syn big} & Bundle &  \\
		 ...
		 & fminbnd &   \\
	\end{tabular}
	\caption[steps]{steps and Null steps}
\end{table}
\end{center}
