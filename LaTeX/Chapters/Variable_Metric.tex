...
\begin{equation*}
		Q(\hat{x}^k):=Q_k = Q_k^{\top} \quad \text{and} \quad -q\mathbb{I} \prec Q_k \prec q\mathbb{I} \text{ for a } q  > 0.
\end{equation*}
...
\begin{enumerate}
	...
	\noindent\hspace*{2em}%
	Set \(\hat{x}^{k+1} = x^{k+1}, \hat{f}^{k+1} = f^{k+1}\) and select \(t_{k+1} > t_{min}\). \\
	Calculate a symmetric matrix \(Q_{k+1}\) with \(-q\mathbb{I} \prec Q_{k+1} \prec q\mathbb{I}\) and adjust \(t_{k+1}\) such that \(Q_{k+1}+\frac{1}{t_k}\mathbb{I} \succ 0\).\\
...
As the algorithm assures, that \(-q\mathbb{I} \prec Q_k \prec q\mathbb{I}\) for some \(q > 0\), the step size can be bounded from below by \(t_{min} = \frac{1}{q} - \varepsilon \) for a small \(\varepsilon > 0\). This means, the matrix \(Q_k+\frac{1}{t_k}\mathbb{I}\) is uniformly bounded for all \(k\). Because the matrix is also symmetric it can be used to define a scalar product which induces indeed the norm \(\Vert \cdot \Vert_{Q_k+\frac{1}{t_k}\mathbb{I}}\).
%\begin{proposition}
...
\label{theo_inf_ser_steps}
	...
	Let algorithm \ref{sec_variable_metric}.1 generate an infinite number of serious steps. Then \(\delta_k \to 0\) as \(k \to \infty\). 
	Let the sequence \(\{\eta_k\}\) be bounded. As \(k \to \infty\) we have \(C_k \to 0\), and for every accumulation point \(\bar{x}\) of \(\{\hat{x}^k\}\) there exists \(\bar{S}\) such that \(S^k \to \bar{S}\) and \(S^k + \nu^k \to 0\).
...
From \(\{\hat{x}^{k'}\}_{k' \in K'} \to \bar{x}\)  follows that  \(d^{\hat{k'}} = \hat{x}^{\hat{k'}+1}-\hat{x}^{\hat{k'}} \to 0\) for \(\hat{K'} \ni \hat{k'} \to \infty \). The step size \(t_k\) is bounded below by \(t_{min} > 0\) so \(\liminf_{k \to \infty} t_k > 0\). Because of this and because the eigenvalues of \(Q_k\) are bounded the expression 
	...
	\[ \lVert S^{\hat{k'}} + \nu^{\hat{k'}} \rVert = \left\Vert\left(Q_{\hat{k'}}+\frac{1}{t_{\hat{k'}}}\mathbb{I}\right)d^{\hat{k'}} \right\Vert \leq \underbrace{\left(\lVert Q_{\hat{k'}}\rVert+\frac{1}{t_{\hat{k'}}}\right)}_{\text{bounded}}\underbrace{\|d^{\hat{k'}}\|}_{\to 0}.\]
...
Defining \(s:= \max_{k,j}\|s^k_j\|\) this yields that 
		...
		By the Bolzano-Weierstrass theorem \cite[p. 51]{Koenigsberger2003} every bounded sequence has a convergent subsequence. Let \(K \subset \hat{K'}\) denote the index set of this converging subsequence and \(\bar{S}\) the corresponding accumulation point. Then finally \(S^k \to \bar{S}\) for \(K \ni k\to \infty\).
...
As \(Q\) is fixed in null steps and  \(\liminf_{k \to \infty} t_k > 0\) (because \(t_k > t_{min} >0\) for all \(k\)) the sequence \(\{\Psi_k\}\) is increasing and bounded from above. It is therefore convergent.
...
\begin{align*}
	...
	&\geq \frac{1}{2}{\lVert d^{k+1}-d^k\rVert}^2_{Q+\frac{1}{t_{\bar{k}}}\mathbb{I}}.
\end{align*}
...
\begin{equation*}
	\lim_{k \to \infty} \delta_k = \lim_{k \to \infty} C_k + \Langle d^k,\left(Q_k+\frac{1}{t_k}\mathbb{I}\right)d^k\Rangle = 0.
\end{equation*}

By the fact that \(C_k \geq 0\) for all \(k\) and positive definiteness of \(Q_k+\frac{1}{t_k}\mathbb{I} \)  it follows that all summands above are nonnegative and hence \(C_k \to 0\) as \(k \to \infty\). Since \(Q+\frac{1}{t_k}\mathbb{I}\) is also bounded due to \(\liminf_{k \to \infty} t_k > 0\) and the bounded eigenvalues of \(Q\)  we have 

\begin{equation*}
	  \lVert d^k\rVert^2_{Q+\frac{1}{t_k}\mathbb{I}} \geq c\Vert d^k\Vert^2 \to 0
\end{equation*}
...
\[ \Vert S^{{k}} + \nu^{{k}} \Vert = \left\Vert\left(Q_{k}+\frac{1}{t_{{k}}}\mathbb{I}\right)d^{{k}} \right\Vert \leq \underbrace{\left(\Vert Q_{{k}}\Vert+\frac{1}{t_{{k}}}\right)}_{\text{bounded}}\underbrace{\Vert d^{k}\Vert}_{\to 0} \to 0.\]
...
\begin{remark}
	In case the matrix \(Q_k\) is also updated in null steps the proof still holds as long as the assumptions on boundedness of \(Q_k\) and positive definiteness of \(Q_k+\frac{1}{t_k}\mathbb{I}\) are still valid.
\end{remark}
