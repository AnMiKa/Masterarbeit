...
The function \(f\) has for example finite directional derivatives in \(x_{{kink}}\), if it is convex on \(U\) \cite[p. 144]{Stoer1970}.
...
%Let a kink be at \(x_{{kink}} \in \R\) and the left sided limiting value of the derivative be \(f'(x_{{kink}}) = a\) the right-sided one \(f'(x_{{kink}}) = b \neq a\). Because \(f\) was assumed to be locally Lipschitz both limits exist and are finite.
...
%\[ \lim_{h \searrow 0} \frac{f'(x_{{kink}}+h)-f'(x_{{kink}})}{h} = \frac{\overbrace{f'(x_{{kink}}+h)}^{\to b}-a}{h} = \pm t\infty, \]
...
%
%color{red}{write only the thing below; write things above as comment in third part of this section???}
...
%color{red}{remark that convergence can depend a lot on the threshold and how the matrix/eigenvalue is scaled in the two methods}
