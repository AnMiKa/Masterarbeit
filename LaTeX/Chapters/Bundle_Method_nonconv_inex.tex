...
then \(f_x\) approximates the value \(f(x)\) within \(\sigma_x\). This is slightly different from the definition in (\ref{conv_inexactness_1}). In the convex case it follows from (\ref{sig_p_thet_subgr}) that \( \bar{\sigma} \geq \sigma_x \geq -\theta_x \geq - \bar{\theta}\) and therefore \(f_x \in [f(x)-\bar{\theta},f(x)+\bar{\sigma}]\).
...
\begin{align}
	&M_{k+1}(\hat{x}^k+d) \geq \hat{f}_{k+1}-c^{k+1}_{k+1}+\left\langle s^{k+1},d \right\rangle \quad \forall d \in \R^n \label{Model_update_1}\\
...
In \cite{Oliveira2014} another stopping criterion is supposed that ensures convergence even for unbounded sequences \(\{\hat{x}^k\}\).
