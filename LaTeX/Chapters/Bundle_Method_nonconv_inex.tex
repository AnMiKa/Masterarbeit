...
For exact function and subgradient information convergence of a very similar method to algorithm \ref{sec_nonconv_inex}.1 is proven also with use of the aggregation technique in \cite{Hare2010}.
...
Here only the missing links are sketched to extend the proof to this case.
...
Without loss of generality we can assume that there is only one aggregate subgradient in the bundle. (If there are more, this results merely in more aggregate terms of the same form and behavior in the convex sum, so it does not alter the argumentation.)
...
As it is explained in the paper due to the uniform boundedness of the set \(J^k\) it is possible to consider it as some fixed index set (for example \(\{1,...,N\}\)). Unused indices are filled with \(\alpha^k_j=0\). Let then \(J\) be the set of all \(j\in J^k\) such that \(\liminf_{k\to\infty}\alpha^k_j >0\). From item (i) of Lemma \ref{Lemma5} follows then that \(\Vert x^j-\hat{x}^k\Vert \to 0\). Hence also \(\Vert x^j-\bar{x}\Vert \leq \Vert x^j-\hat{x}^k\Vert + \Vert \hat{x}^k-\bar{x}\Vert \to 0 \).
...
With the same argumentation as above follows that in the limit the first tow terms of the second equation in (\ref{calc_lem5_2}) are in \(\partial f(\bar{x})+B_{\bar{\theta}}(0)\) and from the induction assumption follows that also the last term is.
