...
\begin{definition} % as in \cite{Clarke1990}
		The function \(f: \R^n \to \R\) is called \emph{Lipschitz near \(x \in \R^n\)} if there exist \(C \in \R\) and \(\varepsilon > 0\) such that 
...
\begin{definition}
	...
	\[ \partial f(x) = \{g \in \R^n | \langle g,d\rangle \leq f'(x,d) \forall d \in \R^n \}. \]
\end{definition}
...
If \(f\) is a convex function it coincides with the subdifferential \(\partial f\) of \(f\) \cite{Clarke1990}.
...
When bundle methods were first introduced in 1975 by Claude Lemaréchal and Philip Wolfe they were developed to minimize a convex (possibly nonsmooth) function \(f\) for which at least one subgradient at a point can be computed \cite{Mifflin2012}. \\
...
\begin{definition} \cite{Rockafellar2009}
	A function \(f:\R^n \to \bar{\R} = [-\infty, + \infty]\) is called \emph{proper} if \(f(x) < \infty\) for at least one \(x\in \R^n\) and \(f(x) > \infty \forall x \R^n\).
\end{definition}
...
\begin{definition}
\emph{Stationary point}: (for nonsmooth optimization, possibly nonconvex) \(\rightarrow\) see Mifflin ``Modification''? or Kiwiel ``aggregate subgradients'' \\
\end{definition}
...
\begin{itemize}
	...
	\item generalized gradients may only be shifted not tilted? \\
		is this realistic assumption?
		\item ask Simon to \(\varepsilon\)-subdifferentials
	\item Algorithm:
	\begin{itemize}
